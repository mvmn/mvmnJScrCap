...
public class ControlWindow extends JFrame implements WindowListener {
    ...
    public ControlWindow() {
        ...
        btnCaptureFullScreen.addActionListener(
                e -> doCapture(screens[cbxScreen.getSelectedIndex()].getDefaultConfiguration().getBounds()));

        sliderOpacity.addChangeListener(e -> ControlWindow.this.captureRectFrame
                .setOpacity(((float) ControlWindow.this.sliderOpacity.getValue()) / 100));
                ...
                if (images.length > 0) {
                    ...
                    if (!sizesOkToExport) {
                        sizesOkToExport = (JOptionPane.OK_OPTION == JOptionPane.showConfirmDialog(ControlWindow.this,
                                "Frames have different sizes - resulting GIF will be corrupt. Continue anyway?",
                                "Frames sizes mismatch", JOptionPane.OK_CANCEL_OPTION));
                    }
                    if (sizesOkToExport) {
                        final JFileChooser fileChooser = new JFileChooser();
                        if (fileChooser.showSaveDialog(ControlWindow.this) == JFileChooser.APPROVE_OPTION) {
                            final ExportProgressDialog progressDialog = new ExportProgressDialog(ControlWindow.this,
                                    images.length, fileChooser.getSelectedFile().getAbsolutePath());
                            progressDialog.pack();
                            SwingUtil.moveToScreenCenter(progressDialog);
                            progressDialog.setVisible(true);
                            GifExportThread exportThread = new GifExportThread(ControlWindow.this, progressDialog,
                                    images, sliderDelay.getValue(), fileChooser.getSelectedFile(),
                                    cbLoopGif.isSelected());
                            exportThread.start();
                        }
                    }
                } else {
                                    ...
                                    SwingUtilities.invokeLater(new Runnable() {
                                        public void run() {
                                            JOptionPane.showMessageDialog(ControlWindow.this,
                                                    "Error occurred while loading: " + e1.getClass().getSimpleName()
                                                            + " - " + e1.getMessage());
                                        }
                                    });
                                }
        ...
        final JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(tblResults),
            ...
            public void actionPerformed(ActionEvent e) {
                if (currentlyPreviewed != null) {
                    ...
                    if (JFileChooser.APPROVE_OPTION == chooser.showSaveDialog(ControlWindow.this)) {
                        try {
                            ImageIO.write(capture.getImage(), cbxImageFormat.getSelectedItem().toString(),
                                    chooser.getSelectedFile());
                        } catch (IOException e1) {
                            e1.printStackTrace();
                            JOptionPane.showMessageDialog(ControlWindow.this, "Error occurred while saving: "
